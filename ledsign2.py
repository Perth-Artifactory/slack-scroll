...
class LEDSign:
    ...
    def send_to_sign(self, msg):
        # byte_string = " ".join("{:02x}".format(ord(c)) for c in msg)
    ...
    def begin_message(self, sign=range(0, 128), reset=False):
        ...
        if type(sign) == int or type(sign) == long:
            ...
            self.send_to_sign("\x00" + chr(sign) + chr(sign))

            if reset:
                self.send_to_sign("\x01")
            else:
                self.send_to_sign("\x00")
    ...
    def set_clock(self, n=None, hour24=True):
        ...
        if hour24:
            ...
            hour24 = "\x01"
        args = (pack("!B", n.isoweekday() % 7), hour24, n.year % 100, n.month, n.day, n.hour, n.minute, n.second)
        self.send_to_sign(("\x08%s%s" + ("%02d" * 6) + '\xFF') % args)
    ...
    def begin_file(self, file_id):
        ...
        self.send_to_sign("\x01%02d" % file_id)
        self.file_id = file_id

    def end_file(self):
        if self.file_id is None:
            raise Exception("No file is open")
        self.send_to_sign("\xFF\xFF")
        self.file_id = None
